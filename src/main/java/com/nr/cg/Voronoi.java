...
/**
 * Constructor for Voronoi diagram of a vector of sites pvec.
 * Bit "1" sent to the Delaunay constructor tells it not to 
 * delete linehash.
 * 
...
public class Voronoi extends Delaunay {
  ...
  public Voronoi(final Point[] pvec) {
    ...
    for (p=0; p<npts; p++) {
      ...
      else throw new IllegalArgumentException("triangle should contain p");
      jfirst = j;
      ccp = circumcircle(pts[p],pts[i],pts[j]).center;
      while (true) {
        key = hashfn.int64p(i) - hashfn.int64p(p);
        Integer[] k_w = new Integer[1];
        if ( linehash.get(key,k_w,0)== 0 ) throw new IllegalArgumentException("Delaunay is incomplete");
        k = k_w[0];
