 ...
 * repeatedly querying which stored points are within a specified radius
 * of a specified new point.
 * 
...
public class Nearpoints {
  ...
  public Nearpoints(final int dim, final Point[] pvec){
    ...
    npts = pvec.length;
    thetree = new Qotree(dim, npts,npts,32/DIM); // Set the tree's outer box and store all the points.
    int j,k;
    ...
    for (j=0; j<npts; j++) thetree.qostore(new Sphcirc(pvec[j],0.0));
  }
  ...
  /**
   * 
   * Once the tree is constructed, this function can be called repeatedly 
   * with varying points pt and radii r. It returns n, the number of stored
   * points within radius r of pt (but no larger than nmax), and copies those
   * points into list[0..n-1].
   * 
   * @param pt
   * @param r
   * @param list
   ...
   * @return
   */
  public int locatenear(final Point pt, final double r, final Point[] list, final int nmax) {
    int j,n;
