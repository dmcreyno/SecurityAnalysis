...
public class Circle {
  ...
  public static Circle circumcircle(final Point a, final Point b, final Point c) {
    ...
    det = a0*c1 - c0*a1;
    if (det == 0.0) throw new IllegalArgumentException("no circle thru colinear points");
    det = 0.5/det;
    asq = a0*a0 + a1*a1;
    csq = c0*c0 + c1*c1;
    ...
    return new Circle(new Point(DIM, new double[]{ctr0 + b.x[0], ctr1 + b.x[1]}), sqrt(rad2));
  }
}
