...
public class Triel {
  ...
  /**
   * Return 1 if point is in the triangle, 0 if on boundary, 1 if 
   * outside. (CCW triangle is assumed.)
   * 
   * @param point
   * @return
   */
   ...
   * @return
   */
  public static double incircle(final Point d, final Point a, final Point b, final Point c) {
    if(d.dim()!=DIM|| a.dim()!=DIM||c.dim()!=DIM)
      throw new IllegalArgumentException("Need same dim!");
    Circle cc = Circle.circumcircle(a,b,c);
    double radd = SQR(d.x[0]-cc.center.x[0]) + SQR(d.x[1]-cc.center.x[1]);
