...
public class Bessel {
  ...
  // private static final double[] c1= new double[NUSE1],c2 = new double[NUSE2];
  private static final double c1[] = {-1.142022680371168e0,6.5165112670737e-3,
  ...
  public void besseljy(final double nu, final double x) {
    ...
    if (x <= 0.0 || nu < 0.0) throw new IllegalArgumentException("bad arguments in besseljy");
    ...
    if (i >= MAXIT)
      throw new IllegalArgumentException("x too large in besseljy; try asymptotic expansion");
    rjl=isign*FPMIN;
    rjpl=h*rjl;
    rjl1=rjl;
    rjp1=rjpl;
    ...
    if (x < XMIN) {
      ...
      if (i > MAXIT) throw new IllegalArgumentException("bessy series failed to converge");
      rymu = -sum;
      ry1 = -sum1*xi2;
      ...
      if (i >= MAXIT) throw new IllegalArgumentException("cf2 failed in besseljy");
      gam=(p-f)/q;
  ...
  public void besselik(final double nu, final double x) {
    ...
    int i,l,nl;
    if (x <= 0.0 || nu < 0.0) throw new IllegalArgumentException("bad arguments in besselik");
    nl=(int)(nu+0.5);
    xmu=nu-nl;
    xmu2=xmu*xmu;
    xi=1.0/x;
    ...
    if (i >= MAXIT)
      throw new IllegalArgumentException("x too large in besselik; try asymptotic expansion");
    ril=FPMIN;
    ripl=h*ril;
    ril1=ril;
    rip1=ripl;
    ...
    if (x < XMIN) {
      ...
      if (i > MAXIT) throw new IllegalArgumentException("bessk series failed to converge");
      rkmu=sum;
      rk1=sum1*xi2;
    } else {
      b=2.0*(1.0+x);
      ...
      if (i >= MAXIT) throw new IllegalArgumentException("besselik: failure to converge in cf2");
      h=a1*h;
  ...
  public void sphbes(final int n, final double x) {
    ...
    double factor,order;
    if (n < 0 || x <= 0.0) throw new IllegalArgumentException("bad arguments in sphbes");
    order=n+0.5;
    besseljy(order,x);
