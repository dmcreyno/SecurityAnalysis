...
public class Elliptic {
  ...
  public static double rc(final double x, final double y) {
    final double ERRTOL=0.0012, THIRD=1.0/3.0, C1=0.3, C2=1.0/7.0,
      C3=0.375, C4=9.0/22.0;
    final double TINY=5.0*Double.MIN_NORMAL,
      BIG=0.2*Double.MAX_VALUE, COMP1=2.236/sqrt(TINY),
      COMP2=SQR(TINY*BIG)/25.0;
    double alamb,ave,s,w,xt,yt;
    if (x < 0.0 || y == 0.0 || (x+abs(y)) < TINY || (x+abs(y)) > BIG ||
      (y<-COMP1 && x > 0.0 && x < COMP2)) throw new IllegalArgumentException("invalid arguments in rc");
    if (y > 0.0) {
      xt=x;
      yt=y;
      w=1.0;
  ...
  public static double rd(final double x, final double y, final double z) {
    final double ERRTOL=0.0015, C1=3.0/14.0, C2=1.0/6.0, C3=9.0/22.0,
      C4=3.0/26.0, C5=0.25*C3, C6=1.5*C4;
    final double TINY=2.0*pow(Double.MAX_VALUE,-2./3.),
      BIG=0.1*ERRTOL*pow(Double.MIN_NORMAL,-2./3.);
  ...
  public static double rf(final double x, final double y, final double z) {
    final double ERRTOL=0.0025, THIRD=1.0/3.0,C1=1.0/24.0, C2=0.1,
      C3=3.0/44.0, C4=1.0/14.0;
    final double TINY=5.0*Double.MIN_NORMAL,
      BIG=0.2*Double.MAX_VALUE;
    double alamb,ave,delx,dely,delz,e2,e3,sqrtx,sqrty,sqrtz,xt,yt,zt;
    if (min(min(x,y),z) < 0.0 || min(min(x+y,x+z),y+z) < TINY ||
      max(max(x,y),z) > BIG) throw new IllegalArgumentException("invalid arguments in rf");
    xt=x;
    yt=y;
    zt=z;
    do {
  ...
  public static double rj(final double x, final double y, final double z, final double p) {
    final double ERRTOL=0.0015, C1=3.0/14.0, C2=1.0/3.0, C3=3.0/22.0,
      C4=3.0/26.0, C5=0.75*C3, C6=1.5*C4, C7=0.5*C2, C8=C3+C3;
    final double TINY=pow(5.0*Double.MIN_NORMAL,1./3.),
      BIG=0.3*pow(0.2*Double.MAX_VALUE,1./3.);
    ...
    if (min(min(x,y),z) < 0.0 || min(min(x+y,x+z),min(y+z,abs(p))) < TINY
      || max(max(x,y),max(z,abs(p))) > BIG) throw new IllegalArgumentException("invalid arguments in rj");
    sum=0.0;
    fac=1.0;
    if (p > 0.0) {
      xt=x;
  ...
  public static double ellpi(final double phi, final double en, final double ak) {
    double cc,enss,q,s;
    s=sin(phi);
    enss=en*s*s;
    cc=SQR(cos(phi));
  ...
  public static void sncndn(final double uu, final double emmc, final doubleW sn, final doubleW cn, final doubleW dn) {
