...
public class Betadist extends Beta{
  ...
  public Betadist(double aalph, double bbet) {
    alph = aalph;
    bet = bbet;
    if (alph <= 0. || bet <= 0.) throw new IllegalArgumentException("bad alph,bet in Betadist");
    fac = gammln(alph+bet)-gammln(alph)-gammln(bet);
  }
  
  public double p(double x) {
    if (x <= 0. || x >= 1.) throw new IllegalArgumentException("bad x in Betadist");
  ...
  public double invcdf(double p) {
    if (p < 0. || p > 1.) throw new IllegalArgumentException("bad p in Betadist");
