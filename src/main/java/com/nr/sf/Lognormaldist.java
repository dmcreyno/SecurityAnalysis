...
public class Lognormaldist extends Erf{
  ...
  public Lognormaldist(final double mmu, final double ssig) {
    mu = mmu;
    sig = ssig;
    if (sig <= 0.) throw new IllegalArgumentException("bad sig in Lognormaldist");
  }
  
  public double p(final double x) {
    if (x < 0.) throw new IllegalArgumentException("bad x in Lognormaldist");
    if (x == 0.) return 0.;
  ...
  public double invcdf(final double p) {
    if (p <= 0. || p >= 1.) throw new IllegalArgumentException("bad p in Lognormaldist");
