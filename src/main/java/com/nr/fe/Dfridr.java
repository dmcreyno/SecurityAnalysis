...
public class Dfridr {
   ...
   * @return
   */
  public static double dfridr(final UniVarRealValueFun func, final double x, final double h, final doubleW err) {
    final int ntab=10;
    ...
    if (h == 0.0) throw new IllegalArgumentException("h must be nonzero in dfridr.");
    hh=h;
    ...
    for (i=1;i<ntab;i++) {
      ...
      if (abs(a[i][i]-a[i-1][i-1]) >= safe*err.val) break;
    }
    return ans;
  }
}
