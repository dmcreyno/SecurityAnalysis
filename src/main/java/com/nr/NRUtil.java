...
public class NRUtil {
  ...
  public static double[] buildVector(final double[] b) {
    ...
    System.arraycopy(b, 0, v, 0, v.length);
    return v;
  }
  ...
  public static void copyAssign(final double[]v, final double[] rhs) {
    // post-condition: normal assignment via copying has been performed;
    ...
    if (v != rhs) {
      /*
      // DLM - originally these inline comments were
      // part of the following if statement.
      // nn=rnn;
      //v = new double[nn];
       */
      if (nn != rnn ) throw new IllegalArgumentException("Must be same size.");
  ...
  public static int[][] buildMatrix(final int nn, final int mm, final int[] b) {
    int[][] v = new int[nn][mm];
    for(int i=0; i<nn; i++)
      System.arraycopy(b, i * mm + 0, v[i], 0, mm);
    return v;
  }
  ...
  public static double[][] buildMatrix(final double[][] b) {
    ...
    double[][] v = new double[nn][mm];
    for(int i=0; i<nn; i++)
      System.arraycopy(b[i], 0, v[i], 0, mm);
    return v;
  }
  ...
  public static void copyAssign(final double[][]v, final double[][] rhs) {
    ...
    if (v != rhs) {
      ...
      // v = new double[nn][mm];
      */
      if (nn != rnn || mm != rmm) throw new IllegalArgumentException("Must be same size.");
      for (int i=0; i< nn; i++)
        System.arraycopy(rhs[i], 0, v[i], 0, mm);
    }
  }
