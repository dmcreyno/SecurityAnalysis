...
public class Calendar {
  ...
  public static int julday(final int mm, final int id, final int iyyy) {
    ...
    if (jy == 0) throw new IllegalArgumentException("julday: there is no year zero.");
    if (jy < 0) ++jy;
    if (mm > 2) {
      jm=mm+1;
    } else {
  ...
  public static void caldat(final int julian, final intW mm, final intW id, final intW iyyy) {
    ...
    mm.val=je-1;
    if (mm.val > 12) mm.val -= 12;
    iyyy.val=jc-4715;
    if (mm.val > 2) --iyyy.val;
    if (iyyy.val <= 0) --iyyy.val;
    if (julian < 0) iyyy.val -= 100*(1-julian/36525);
  }
  
  public static void flmoon(final int n, final int nph, final intW jd, final doubleW frac) {
    ...
    else throw new IllegalArgumentException("nph is unknown in flmoon");
