...
public class Arithcode {
  ...
  public Arithcode(final int[] nfreq, final int nnch, final int nnrad) {
    ...
    int j;
    if (nrad > 256) throw new IllegalArgumentException("output radix must be <= 256 in Arithcode");
  ...
  public void codeone(final int ich, final byte[] code, final intW lcd) {
    if (ich > nch) throw new IllegalArgumentException("bad ich in Arithcode");
  ...
  public void advance(final int ich, final byte[] code, final intW lcd, final int isign) {
    int j,k,jh,jl;
    ...
    for (j=nc;j<NWK;j++) {
      ...
      if (isign > 0) code[lcd.val] = (byte)(ilob[j]);
      lcd.val++;
    }
    if (j+1 > NWK) return;
    nc=j;
    for(j=0;jdif<minint;j++)
      jdif *= nrad;
    if (j > nc) throw new IllegalArgumentException("NWK too small in arcode.");
    if (j != 0) {
      for (k=nc;k<NWK;k++) {
        iupb[k-j]=iupb[k];
        ilob[k-j]=ilob[k];
