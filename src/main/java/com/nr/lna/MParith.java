...
public class MParith {
  ...
  private static char lobyte(int x) {return (char)(x & 0xff);}
  private static char hibyte(int x) {return (char)((x >> 8) & 0xff);}
  
  public static void mpmul(char[] w, char[] u, char[] v) {
    final double RX=256.0;
    int j,nn=1,n=u.length,m=v.length,p=w.length,n_max=max(m,n);
    double cy,t;
    while (nn < n_max) nn <<= 1;
    nn <<= 1;
    ...
    for (j=0;j<p;j++) w[j]=0;
    w[0]=(char)(cy);
    for (j=1;j<min(n+m,p);j++) w[j]=(char)(b[j-1]);
  }
  ...
  public static void mpinv(char[] u, char[] v) {
    ...
    for (;;) {
      ...
      mplsh(s);
      mpneg(s);
      s[0] += (char)(2); s[0] &=0xFF; // XXX one char is 2 bytes
      mpmul(r,s,u);
      mplsh(r);
      mpmov(u,r);
      for (j=1;j<n-1;j++)
  ...
  public static void mpdiv(char[] q, char[] r, char[] u, char[] v) {
    final int MACC=1;
    int i,is,mm,n=u.length,m=v.length,p=r.length,n_min=min(m,p);
    if (m > n) throw new IllegalArgumentException("Divisor longer than dividend in mpdiv");
    mm=m+MACC;
    char[] s=new char[mm],rr=new char[mm],ss=new char[mm+1],qq=new char[n-m+1],t=new char[n];
    mpinv(s,v);
    mpmul(rr,s,u);
    mpsad(ss,rr,1);
    mplsh(ss);
    ...
    mplsh(t);
    is=mpsub(t,u,t);
    if (is != 0) throw new IllegalArgumentException("MACC too small in mpdiv");
  ...
  public static void mpsqrt(char[] w, char[] u, char[] v) {
    ...
    char[] r=new char[2*n],x=new char[n+m],s=new char[2*n+m],t=new char[3*n+m];
    double fu,fv=(double)(v[mm-1]);
    for (j=mm-2;j>=0;j--) {
      fv *= BI;
      fv += v[j];
    ...
    for (;;) {
      ...
      mplsh(s);
      mpneg(s);
      s[0] += (char)(3); s[0] &=0xFF; // XXX one char is 2 bytes
      ir=mpsdv(s,s,2);
      for (j=1;j<n-1;j++) {
        if (s[j] != 0) {
          mpmul(t,s,u);
