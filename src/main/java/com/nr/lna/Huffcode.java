...
public class Huffcode {
  ...
  public Huffcode(final int nnch, final int[] nfreq) {
    ...
    if (nlong > INT_DIGITS) // numeric_limits<Uint>::digits
      throw new IllegalArgumentException("Code too long in Huffcode.  See text.");
  }
  ...
  public void codeone(final int ich, final byte[] code, final intW nb) {
    int m,n,nc;
    if (ich >= nch) throw new IllegalArgumentException("bad ich (out of range) in Huffcode");
    if (ncod[ich]==0) throw new IllegalArgumentException("bad ich (zero prob) in Huffcode");
  ...
  public int decodeone(final byte[] code, final intW nb) {
    ...
    for (;;) {
      nc=nb.val >> 3;
      node=((code[nc] & setbit[7 & nb.val++]) != 0 ?
   ...
   * @param m
   */
  public void heep(final int[] index, final int[] nprob, final int n, final int m) {
    int i=m,j,k;
    k=index[i];
    while (i < (n >> 1)) {
      if ((j = 2*i+1) < n-1
