...
public class PSpage {
  ...
  public void setlinewidth(final double w) {PLT.printf("%g setlinewidth\n",w);}

  public void setgray(final double w) {PLT.printf("%g setgray\n",w);}

  public void gsave() {PLT.printf("gsave\n");}

  public void grestore() {PLT.printf("grestore\n");}

  public void rawps(final String text) {PLT.printf("%s\n",text);}

  public void addtext(final String text) { PLT.printf("(%s) show ",text); }

  public void puttext(final String text, final double x, final double y) {
    puttext(text, x, y, 0.0);
  }
  
  public void puttext(final String text, final double x, final double y, final double rot) {
  ...
  public void putctext(final String text, final double x, final double y, final double rot) {
    PLT.printf("gsave %g %g translate %g rotate 0 0 mt (%s) ",x,y,rot,text);
  ...
  public void putrtext(final String text, final double x, final double y, final double rot) {
    PLT.printf("gsave %g %g translate %g rotate 0 0 mt (%s) ",x,y,rot,text);
    PLT.printf("dup stringwidth pop neg 0 rmoveto show grestore\n");
  }

  public void close() {PLT.printf("showpage\n"); PLT.close(); PLT = null;}
  ...
  public void polyline(final double[] x, final double[] y, final boolean close, final boolean fill, final boolean clip) {
    ...
    for (i=1;i<n;i++) PLT.printf("%g %g lt\n",x[i],y[i]);
    if (close || fill || clip) PLT.printf("cp ");
    if (fill) PLT.printf("fi\n");
    else if (clip) PLT.printf("clip\n");
    else PLT.printf("st\n");
  }
}
