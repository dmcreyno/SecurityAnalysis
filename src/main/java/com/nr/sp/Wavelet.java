...
public abstract class Wavelet {
  ...
  public static void wtn(final double[] a, final int[] nn, final int isign, final Wavelet wlet) {
    int idim,i1,i2,i3,k,n,nnew,nprev=1,nt,ntot=1;
    int ndim=nn.length;
    for (idim=0;idim<ndim;idim++) ntot *= nn[idim];
    if ((ntot&(ntot-1))!=0) throw new IllegalArgumentException("all lengths must be powers of 2 in wtn");
