...
public class Solvde {
  ...
  public Solvde(final int itmaxx, final double convv, final double slowcc,
    final double[] scalvv, final int[] indexvv, final int nbb, final double[][] yy,
    final Difeq difeqq) {
    itmax = itmaxx;
    conv = convv;
    slowc=slowcc;
  ...
  public void pinvs(final int ie1, final int ie2, final int je1, final int jsf,
    final int jc1, final int k){
    ...
    for (i=ie1;i<ie2;i++) {
      ...
      if (big == 0.0)
        throw new IllegalArgumentException("Singular matrix - row all 0, in pinvs");
      pscl[i-ie1]=1.0/big;
      indxr[i-ie1]=0;
    }
    for (id=0;id<iesize;id++) {
      ...
      if (s[ipiv][jpiv] == 0.0)
        throw new IllegalArgumentException("Singular matrix in routine pinvs");
  ...
  public void bksub(final int ne, final int nb, final int jf, final int k1, final int k2){
    int nbf=ne-nb,im=1;
