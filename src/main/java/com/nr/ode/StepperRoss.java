...
/**
 * fourth-order stiffly stable Rosenbrock stepper
...
public class StepperRoss extends StepperBase{
  ...
  public static class Controller {
    ...
    public boolean success(final double err, final doubleW h) {
      ...
      if (err <= 1.0) {
        if (!first_step) {
          double facpred=(hold/h.val)*pow(err*err/errold,0.25)/safe;
  ...
  public StepperRoss(final double[] yy, final double[] dydxx, final double xx,
  ...
  public void step(final double htry,final DerivativeInf derivs) {
    ...
    for (;;) {
      ...
      if (abs(h.val) <= abs(x)*EPS)
        throw new IllegalArgumentException("stepsize underflow in StepperRoss");
    }
