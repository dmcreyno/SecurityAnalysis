...
public class StepperDopr5 extends StepperBase{
  ...
  public static class Controller {
    static final double beta=0.0,alpha=0.2-beta*0.75,safe=0.9,minscale=0.2,maxscale=10.0;
    double hnext,errold;
    boolean reject;
  ...
  public StepperDopr5(final double[] yy,final double[] dydxx,final double xx,
      final double atoll,final double rtoll,final boolean dense) {
  ...
  public void step(final double htry, final DerivativeInf derivs) {
    ...
    for (;;) {
      ...
      if (abs(h.val) <= abs(x)*EPS)
        throw new IllegalArgumentException("stepsize underflow in StepperDopr5");
    }
    if (dense)
