...
public class StepperStoerm extends StepperBS{
  ...
  public StepperStoerm(final double[] yy, final double[] dydxx, final double xx,
  ...
  public double dense_out(final int i,final double x,final double h) {
    ...
    int neqn=n/2;
    if (i>=neqn) throw new IllegalArgumentException("no dense output for y' in StepperStoerm");
    double yinterp=dens[i]+theta*(dens[neqn+i]+theta1*(dens[2*neqn+i]+
  ...
  public void dense_interp(final int n, final double[] y, final int imit) {
    double y0,y1,yp0,yp1,ypp0,ypp1,ydiff,ah,bh,ch,dh,eh,fh,gh,abh,gfh,gmf,
