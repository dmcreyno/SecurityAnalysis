...
public class StepperDopr853 extends StepperBase{
  ...
  public static class Controller {
    ...
    public boolean success(final double err, final doubleW h) {
      final double beta=0.0,alpha=1.0/8.0-beta*0.2,safe=0.9,minscale=0.333,
        maxscale=6.0;
      double scale;
      if (err <= 1.0) {
        if (err == 0.0)
  ...
  public StepperDopr853(final double[] yy,final double[] dydxx,final double xx,
  ...
  public void step(final double htry,final DerivativeInf derivs) {
    ...
    for (;;) {
      ...
      if (abs(h.val) <= abs(x)*EPS)
        throw new IllegalArgumentException("stepsize underflow in StepperDopr853");
    }
